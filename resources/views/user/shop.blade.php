...
<section class="py-20">
    <div class="container mx-auto px-6">
        <div class="text-center mb-16">
            <h1 class="text-6xl font-bold text-gray-900 mb-6">Featured Plants</h1>
            <p class="text-gray-600 text-lg max-w-4xl mx-auto leading-relaxed">
                Discover our handpicked selection of beautiful plants that will transform your space into a green sanctuary.
            </p>
        </div>
                ...
                <div class="group bg-white rounded-3xl shadow-2xl overflow-hidden transform hover:scale-105 transition-all duration-500">
                    <div class="relative overflow-hidden">
                        @if($plant->image->count() > 0)
                            <img src="{{ $plant->image->first()->image_url }}" alt="{{ $plant->Plant_Name }}" class="w-full h-64 object-cover group-hover:scale-110 transition-transform duration-500">
                        @else
                    ...
                    <div class="p-8">
                        <h3 class="text-2xl font-bold text-gray-900 mb-2">{{ $plant->Plant_Name }}</h3>
                        <p class="text-gray-600 mb-4">{{ $plant->type->Type_Name ?? 'Indoor Plant' }}</p>
                        <div class="flex justify-between items-center">
                            <p class="text-3xl font-bold text-emerald-600">${{ number_format($plant->Price, 0) }}</p>
                            @if($plant->Stock > 0)
                                <form action="{{ route('user.cart.add') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="plant_id" value="{{ $plant->Plant_ID }}">
                                    <input type="hidden" name="quantity" value="1">
                                    <button type="submit" class="bg-gradient-to-r from-emerald-500 to-teal-500 text-white px-6 py-3 rounded-full hover:from-emerald-600 hover:to-teal-600 transition-all duration-300 font-medium">
                                        Add to Cart
                                    </button>
                                </form>
                            @else
                                <button disabled class="bg-gray-400 text-white px-6 py-3 rounded-full cursor-not-allowed font-medium">
                                    Out of Stock
                                </button>
                            @endif
                        </div>
...
<section class="py-20 bg-gradient-to-br from-teal-600 to-blue-300 text-white">
    <div class="container mx-auto px-6">
        <div class="text-center mb-16">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="bg-white/10 backdrop-blur-md rounded-3xl p-8 text-center">
                <div class="w-20 h-20 bg-gradient-to-br from-teal-500 to-blue-600 rounded-full flex items-center justify-center mx-auto mb-6">
